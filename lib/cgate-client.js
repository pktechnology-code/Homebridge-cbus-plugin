...
const KEEPALIVE_INITIAL_DELAY_MS = 30000;
const MAX_PENDING_COMMANDS = 256;
const MAX_LINE_LENGTH = 1024 * 1024;
const MAX_SNIPPET_LENGTH = 20 * 1024 * 1024;
...
function CBusClient(cgateIpAddress, cgateControlPort,
	...
	this.maxPendingCommands = MAX_PENDING_COMMANDS;
	this.maxLineLength = MAX_LINE_LENGTH;
	this.maxSnippetLength = MAX_SNIPPET_LENGTH;
...
CBusClient.prototype.connect = function (callback) {
	...
	function complete(err) {
		if (that.handshakeTimer) {
			clearTimeout(that.handshakeTimer);
			that.handshakeTimer = undefined;
		}
	...
	function reopenConnection() {
		...
		if (that.disconnectRequested || that.reconnectTimer) {
			return;
		}
		...
		that.emit('status', { state: 'reconnecting', retryInSeconds: delay });
		that.reconnectTimer = setTimeout(function () {
			that.reconnectTimer = undefined;
			that.connect();
		}, delay * 1000);
	}
	...
	const socket = net.createConnection(this.cgateControlPort, this.cgateIpAddress);

	// detect half-open connections (e.g. C-Gate host losing power without
	// closing the socket) so the 'close' handler can trigger a reconnect
	socket.setKeepAlive(true, KEEPALIVE_INITIAL_DELAY_MS);

	this.socket = socket;
	this.handshakeTimer = setTimeout(() => {
		const err = new Error(
			`Timed out waiting for C-Gate handshake at ${that.cgateIpAddress}:${that.cgateControlPort}.`
		);
		complete(err);
		socket.destroy(err);
	...
	socket.on('close', function () {
		...
		that._failPendingRequests(new Error('C-Gate connection closed.'));

		// keep retrying even if the very first attempt failed (e.g. C-Gate
		// starts after Homebridge on boot), but only for the current socket
		if ((that.socket === socket) && !that.disconnectRequested) {
			reopenConnection();
		}
	});
	...
	carrier.carry(socket, function (line) {
		if (that.connectionReady) {
			...
			if (parts = line.match(SERVICE_READY_REGEX)) {
				...
				log(`C-Gate session established and configured at ${that.cgateControlPort}:${that.cgateIpAddress}`);
				that.connectionReady = true;
				that.everReady = true;
				that.emit('status', { state: 'connected', connectedAt: new Date().toISOString() });
				complete();
			} else {
...
CBusClient.prototype.disconnect = function () {
	...
	this._failPendingRequests(new Error('C-Gate client disconnected.'));
	this.socket.destroy();
};
...
CBusClient.prototype._sendMessage = function (message, callback) {
	if (!this.socket || !this.connectionReady || !this.socket.writable) {
		this._notifyRequestError({ message, callback }, new Error('C-Gate connection is not ready.'));
		return;
	}
	...
	if (this.pendingCommands.size >= this.maxPendingCommands) {
		this._notifyRequestError(
			{ message, callback },
			new Error(`C-Gate command queue limit of ${this.maxPendingCommands} reached.`)
		);
		return;
	}
	...
	request.timer = setTimeout(() => {
		if (this.pendingCommands.delete(nextCommandId)) {
			this._notifyRequestError(
				request,
				new Error(`C-Gate command ${nextCommandId} timed out after ${this.commandTimeoutMs}ms.`)
			);
		}
	...
	this.socket.write(request.raw + `\n`, err => {
		if (err) {
			...
			if (this.pendingCommands.delete(nextCommandId)) {
				...
				this._notifyRequestError(request, err);
			}
		} else {
...
CBusClient.prototype._notifyRequestError = function (request, error) {
	...
	this.emit('commandError', error, request);
};

CBusClient.prototype._failPendingRequests = function (error) {
	const requests = [...this.pendingCommands.values()];
	this.pendingCommands.clear();

	for (const request of requests) {
		clearTimeout(request.timer);
		this._notifyRequestError(request, error);
	}
};
...
CBusClient.prototype._resolveSnippetFragment = function (fragment) {
	...
	if (fragment.code === 343) {
		...
		const currentLength = this.snippet.content ? this.snippet.content.length : 0;
		if (currentLength + fragment.remainder.length > this.maxSnippetLength) {
			this.snippet = undefined;
			throw new Error(`C-Gate XML snippet exceeded ${this.maxSnippetLength} bytes`);
		}
...
CBusClient.prototype._socketReceivedLine = function (line) {
	if (typeof line !== 'string' || line.length > this.maxLineLength) {
		const err = new Error(`C-Gate line exceeded ${this.maxLineLength} bytes`);
