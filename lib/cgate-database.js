...
const MAX_DATABASE_XML_LENGTH = 20 * 1024 * 1024;
...
CGateDatabase.prototype.isLoaded = function () {
	return typeof this.applications !== `undefined`;
};
...
CGateDatabase.prototype.fetch = function (client, callback) {
	...
	client.getDB(this.netId, result => {
		if (!result || !result.snippet || typeof result.snippet.content !== `string`) {
		const details = result && result.error
			? ` ${result.error.message}`
			: result && result.raw
				? ` C-Gate replied: ${result.raw}`
				: ``;
			if (callback) {
				callback(new Error(`C-Gate did not return database XML for ${this.netId}.${details}`));
			}
			return;
		}

		const dbxml = result.snippet.content;
		if (dbxml.length > MAX_DATABASE_XML_LENGTH) {
			if (callback) {
				callback(new Error(`C-Gate database XML exceeded ${MAX_DATABASE_XML_LENGTH} bytes.`));
			}
			return;
		}
