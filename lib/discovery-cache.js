...
function readCachedGroupCount(filePath) {
	try {
		const existing = JSON.parse(fs.readFileSync(filePath, 'utf8'));
		return Array.isArray(existing.groups) ? existing.groups.length : 0;
	} catch (err) {
		return 0;
	}
}
...
function writeDiscoveryCache(platform) {
	...
	// an empty C-Gate database usually means the C-Bus network is still coming
	// up after a power cycle -- keep the previous discovery results
	if (cache.totalGroups === 0 && readCachedGroupCount(exportPath) > 0) {
		return {
			path: exportPath,
			totalGroups: 0,
			skipped: true
