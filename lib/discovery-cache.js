...
function suggestType(group) {
	...
	if (inferred.includes('dimmer')) {
		return 'dimmer';
	}

	if (inferred.includes('fan')) {
		return 'fan';
	}

	if (inferred.includes('shutter') || inferred.includes('blind') || inferred.includes('curtain')) {
		return 'shutter';
	}

	if (inferred.includes('motion')) {
		return 'motion';
	}

	if (inferred.includes('smoke')) {
		return 'smoke';
	}

	if (inferred.includes('contact')) {
		return 'contact';
	}

	if (inferred.includes('temperature')) {
		return 'temperature';
	}
	...
	if (inferred.includes('relay') || inferred.includes('light') || inferred.includes('lighting')) {
		return 'light';
	}
...
function resolveExportPath(config) {
	...
	return path.resolve(process.cwd(), 'cbus-discovery-cache.json');
}
...
function writeDiscoveryCache(platform) {
	const exportPath = resolveExportPath(platform.config || {});
